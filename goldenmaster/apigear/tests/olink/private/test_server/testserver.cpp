...
void TestServer::onNewConnection()
{
    ...
    m_socket->connect(m_socket.get(), &QWebSocket::textMessageReceived, [this](auto message){ handleMessage(message);});
}
...
void TestServer::handleMessage(const QString &msg)
{
    m_storage.receiveMessage(msg);
}
