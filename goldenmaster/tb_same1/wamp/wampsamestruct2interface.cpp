...
{    
    ...
    WampClient::instance()->doCall("tb.same1.SameStruct2Interface._get", json::array(), json::object(), [this](WAMP::ResponseArg arg) {
        this->applyState(arg.kwargs);
    });

    // subscribe to state changes
    WampClient::instance()->doSubscribe("tb.same1.SameStruct2Interface", [this](WAMP::EventArg arg) {
        this->applyState(arg.kwargs);
    });
    // subscribe to signal sig1
    WampClient::instance()->doSubscribe("tb.same1.SameStruct2Interface.sig1", [this](WAMP::EventArg arg) {
        this->notifySig1(arg);
    });
    // subscribe to signal sig2
    WampClient::instance()->doSubscribe("tb.same1.SameStruct2Interface.sig2", [this](WAMP::EventArg arg) {
        this->notifySig2(arg);
    });
}
...
void WampSameStruct2Interface::setProp1(const Struct2& prop1)
{
    ...
    WampClient::instance()->doCall("tb.same1.SameStruct2Interface._set", json::array(), fields);
}
...
QtPromise::QPromise<Struct1> WampSameStruct2Interface::func1Async(const Struct1& param1)
{
    return QtPromise::QPromise<Struct1>{[&](
        const QtPromise::QPromiseResolve<Struct1>& resolve) {
            WampClient::instance()->doCall("tb.same1.SameStruct2Interface.func1", json::array({param1}), json::object(), [resolve](WAMP::ResponseArg arg) {                
...
QtPromise::QPromise<Struct1> WampSameStruct2Interface::func2Async(const Struct1& param1, const Struct2& param2)
{
    return QtPromise::QPromise<Struct1>{[&](
        const QtPromise::QPromiseResolve<Struct1>& resolve) {
            WampClient::instance()->doCall("tb.same1.SameStruct2Interface.func2", json::array({param1,param2}), json::object(), [resolve](WAMP::ResponseArg arg) {                
                const Struct1& result = arg.args[0].get<Struct1>();
                resolve(result);
            });
        }
