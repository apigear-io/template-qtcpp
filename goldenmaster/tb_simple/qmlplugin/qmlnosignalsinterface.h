...
/**
* A QML wrapper of the NoSignalsInterface implementation.
* Uses a NoSignalsInterface backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by NoSignalsInterface backend or
* invoke operations on it. Have in mind that singals provided by NoSignalsInterface backend should be
* emitted only by NoSignalsInterface backend, emitting it on qml will not reach the NoSignalsInterface backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class TB_SIMPLE_QML_EXPORT QmlNoSignalsInterface : public AbstractNoSignalsInterface
{
    Q_OBJECT
    QML_NAMED_ELEMENT(TbSimpleNoSignalsInterface)
...
public:
    explicit QmlNoSignalsInterface(QObject *parent = nullptr);
    ~QmlNoSignalsInterface() override;
    /**
...
private:
    /**
    * Backend of AbstractNoSignalsInterface type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractNoSignalsInterface> m_obj;
};
