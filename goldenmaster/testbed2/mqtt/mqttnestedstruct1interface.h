...
/**
* Adapts the general Mqtt Client handler to a NestedStruct1Interface in a way it provides access
...
*/
class TESTBED2_MQTT_LIBRARY_EXPORT MqttNestedStruct1Interface : public AbstractNestedStruct1Interface
{
    Q_OBJECT
public:
    /** ctor */
    explicit MqttNestedStruct1Interface(ApiGear::Mqtt::Client& client, QObject *parent = nullptr);
    /** dtor */
    ~MqttNestedStruct1Interface() override;
    /**
    * Property getter
...
private:
    // Helper function, subscribes for property changes from the remote NestedStruct1Interface service.
    void subscribeForPropertiesChanges();
    // Helper function, subscribes for signals emitted by the remote NestedStruct1Interface service.
    void subscribeForSignals();
    // Helper function, subscribes for result of invoke on remote NestedStruct1Interface service.
