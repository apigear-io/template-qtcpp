...
/**
* A QML wrapper of the TestApi0 implementation.
* Uses a TestApi0 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi0 backend or
* invoke operations on it. Have in mind that singals provided by TestApi0 backend should be
* emitted only by TestApi0 backend, emitting it on qml will not reach the TestApi0 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi0 : public AbstractTestApi0
{
    Q_OBJECT
...
public:
    explicit QmlTestApi0(QObject *parent = nullptr);
    ~QmlTestApi0() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi0 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi0> m_obj;
};
