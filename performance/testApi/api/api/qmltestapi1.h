...
/**
* A QML wrapper of the TestApi1 implementation.
* Uses a TestApi1 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi1 backend or
* invoke operations on it. Have in mind that singals provided by TestApi1 backend should be
* emitted only by TestApi1 backend, emitting it on qml will not reach the TestApi1 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi1 : public AbstractTestApi1
{
    Q_OBJECT
...
public:
    explicit QmlTestApi1(QObject *parent = nullptr);
    ~QmlTestApi1() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi1 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi1> m_obj;
};
