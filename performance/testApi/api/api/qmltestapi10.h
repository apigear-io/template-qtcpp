...
/**
* A QML wrapper of the TestApi10 implementation.
* Uses a TestApi10 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi10 backend or
* invoke operations on it. Have in mind that singals provided by TestApi10 backend should be
* emitted only by TestApi10 backend, emitting it on qml will not reach the TestApi10 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi10 : public AbstractTestApi10
{
    Q_OBJECT
...
public:
    explicit QmlTestApi10(QObject *parent = nullptr);
    ~QmlTestApi10() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi10 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi10> m_obj;
};
