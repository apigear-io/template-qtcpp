...
/**
* A QML wrapper of the TestApi11 implementation.
* Uses a TestApi11 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi11 backend or
* invoke operations on it. Have in mind that singals provided by TestApi11 backend should be
* emitted only by TestApi11 backend, emitting it on qml will not reach the TestApi11 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi11 : public AbstractTestApi11
{
    Q_OBJECT
...
public:
    explicit QmlTestApi11(QObject *parent = nullptr);
    ~QmlTestApi11() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi11 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi11> m_obj;
};
