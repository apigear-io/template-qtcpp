...
/**
* A QML wrapper of the TestApi12 implementation.
* Uses a TestApi12 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi12 backend or
* invoke operations on it. Have in mind that singals provided by TestApi12 backend should be
* emitted only by TestApi12 backend, emitting it on qml will not reach the TestApi12 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi12 : public AbstractTestApi12
{
    Q_OBJECT
...
public:
    explicit QmlTestApi12(QObject *parent = nullptr);
    ~QmlTestApi12() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi12 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi12> m_obj;
};
