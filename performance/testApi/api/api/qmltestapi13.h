...
/**
* A QML wrapper of the TestApi13 implementation.
* Uses a TestApi13 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi13 backend or
* invoke operations on it. Have in mind that singals provided by TestApi13 backend should be
* emitted only by TestApi13 backend, emitting it on qml will not reach the TestApi13 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi13 : public AbstractTestApi13
{
    Q_OBJECT
...
public:
    explicit QmlTestApi13(QObject *parent = nullptr);
    ~QmlTestApi13() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi13 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi13> m_obj;
};
