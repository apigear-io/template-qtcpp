...
/**
* A QML wrapper of the TestApi14 implementation.
* Uses a TestApi14 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi14 backend or
* invoke operations on it. Have in mind that singals provided by TestApi14 backend should be
* emitted only by TestApi14 backend, emitting it on qml will not reach the TestApi14 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi14 : public AbstractTestApi14
{
    Q_OBJECT
...
public:
    explicit QmlTestApi14(QObject *parent = nullptr);
    ~QmlTestApi14() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi14 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi14> m_obj;
};
