...
/**
* A QML wrapper of the TestApi15 implementation.
* Uses a TestApi15 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi15 backend or
* invoke operations on it. Have in mind that singals provided by TestApi15 backend should be
* emitted only by TestApi15 backend, emitting it on qml will not reach the TestApi15 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi15 : public AbstractTestApi15
{
    Q_OBJECT
...
public:
    explicit QmlTestApi15(QObject *parent = nullptr);
    ~QmlTestApi15() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi15 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi15> m_obj;
};
