...
/**
* A QML wrapper of the TestApi16 implementation.
* Uses a TestApi16 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi16 backend or
* invoke operations on it. Have in mind that singals provided by TestApi16 backend should be
* emitted only by TestApi16 backend, emitting it on qml will not reach the TestApi16 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi16 : public AbstractTestApi16
{
    Q_OBJECT
...
public:
    explicit QmlTestApi16(QObject *parent = nullptr);
    ~QmlTestApi16() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi16 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi16> m_obj;
};
