...
/**
* A QML wrapper of the TestApi17 implementation.
* Uses a TestApi17 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi17 backend or
* invoke operations on it. Have in mind that singals provided by TestApi17 backend should be
* emitted only by TestApi17 backend, emitting it on qml will not reach the TestApi17 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi17 : public AbstractTestApi17
{
    Q_OBJECT
...
public:
    explicit QmlTestApi17(QObject *parent = nullptr);
    ~QmlTestApi17() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi17 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi17> m_obj;
};
