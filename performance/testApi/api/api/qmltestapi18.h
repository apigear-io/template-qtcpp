...
/**
* A QML wrapper of the TestApi18 implementation.
* Uses a TestApi18 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi18 backend or
* invoke operations on it. Have in mind that singals provided by TestApi18 backend should be
* emitted only by TestApi18 backend, emitting it on qml will not reach the TestApi18 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi18 : public AbstractTestApi18
{
    Q_OBJECT
...
public:
    explicit QmlTestApi18(QObject *parent = nullptr);
    ~QmlTestApi18() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi18 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi18> m_obj;
};
