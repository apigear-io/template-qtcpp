...
/**
* A QML wrapper of the TestApi19 implementation.
* Uses a TestApi19 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi19 backend or
* invoke operations on it. Have in mind that singals provided by TestApi19 backend should be
* emitted only by TestApi19 backend, emitting it on qml will not reach the TestApi19 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi19 : public AbstractTestApi19
{
    Q_OBJECT
...
public:
    explicit QmlTestApi19(QObject *parent = nullptr);
    ~QmlTestApi19() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi19 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi19> m_obj;
};
