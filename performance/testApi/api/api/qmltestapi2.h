...
/**
* A QML wrapper of the TestApi2 implementation.
* Uses a TestApi2 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi2 backend or
* invoke operations on it. Have in mind that singals provided by TestApi2 backend should be
* emitted only by TestApi2 backend, emitting it on qml will not reach the TestApi2 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi2 : public AbstractTestApi2
{
    Q_OBJECT
...
public:
    explicit QmlTestApi2(QObject *parent = nullptr);
    ~QmlTestApi2() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi2 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi2> m_obj;
};
