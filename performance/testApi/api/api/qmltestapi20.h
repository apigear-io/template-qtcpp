...
/**
* A QML wrapper of the TestApi20 implementation.
* Uses a TestApi20 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi20 backend or
* invoke operations on it. Have in mind that singals provided by TestApi20 backend should be
* emitted only by TestApi20 backend, emitting it on qml will not reach the TestApi20 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi20 : public AbstractTestApi20
{
    Q_OBJECT
...
public:
    explicit QmlTestApi20(QObject *parent = nullptr);
    ~QmlTestApi20() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi20 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi20> m_obj;
};
