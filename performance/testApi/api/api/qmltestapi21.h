...
/**
* A QML wrapper of the TestApi21 implementation.
* Uses a TestApi21 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi21 backend or
* invoke operations on it. Have in mind that singals provided by TestApi21 backend should be
* emitted only by TestApi21 backend, emitting it on qml will not reach the TestApi21 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi21 : public AbstractTestApi21
{
    Q_OBJECT
...
public:
    explicit QmlTestApi21(QObject *parent = nullptr);
    ~QmlTestApi21() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi21 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi21> m_obj;
};
