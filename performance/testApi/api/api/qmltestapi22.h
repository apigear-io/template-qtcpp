...
/**
* A QML wrapper of the TestApi22 implementation.
* Uses a TestApi22 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi22 backend or
* invoke operations on it. Have in mind that singals provided by TestApi22 backend should be
* emitted only by TestApi22 backend, emitting it on qml will not reach the TestApi22 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi22 : public AbstractTestApi22
{
    Q_OBJECT
...
public:
    explicit QmlTestApi22(QObject *parent = nullptr);
    ~QmlTestApi22() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi22 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi22> m_obj;
};
