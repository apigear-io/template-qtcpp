...
/**
* A QML wrapper of the TestApi23 implementation.
* Uses a TestApi23 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi23 backend or
* invoke operations on it. Have in mind that singals provided by TestApi23 backend should be
* emitted only by TestApi23 backend, emitting it on qml will not reach the TestApi23 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi23 : public AbstractTestApi23
{
    Q_OBJECT
...
public:
    explicit QmlTestApi23(QObject *parent = nullptr);
    ~QmlTestApi23() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi23 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi23> m_obj;
};
