...
/**
* A QML wrapper of the TestApi24 implementation.
* Uses a TestApi24 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi24 backend or
* invoke operations on it. Have in mind that singals provided by TestApi24 backend should be
* emitted only by TestApi24 backend, emitting it on qml will not reach the TestApi24 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi24 : public AbstractTestApi24
{
    Q_OBJECT
...
public:
    explicit QmlTestApi24(QObject *parent = nullptr);
    ~QmlTestApi24() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi24 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi24> m_obj;
};
