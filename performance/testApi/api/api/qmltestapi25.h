...
/**
* A QML wrapper of the TestApi25 implementation.
* Uses a TestApi25 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi25 backend or
* invoke operations on it. Have in mind that singals provided by TestApi25 backend should be
* emitted only by TestApi25 backend, emitting it on qml will not reach the TestApi25 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi25 : public AbstractTestApi25
{
    Q_OBJECT
...
public:
    explicit QmlTestApi25(QObject *parent = nullptr);
    ~QmlTestApi25() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi25 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi25> m_obj;
};
