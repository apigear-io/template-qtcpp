...
/**
* A QML wrapper of the TestApi26 implementation.
* Uses a TestApi26 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi26 backend or
* invoke operations on it. Have in mind that singals provided by TestApi26 backend should be
* emitted only by TestApi26 backend, emitting it on qml will not reach the TestApi26 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi26 : public AbstractTestApi26
{
    Q_OBJECT
...
public:
    explicit QmlTestApi26(QObject *parent = nullptr);
    ~QmlTestApi26() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi26 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi26> m_obj;
};
