...
/**
* A QML wrapper of the TestApi27 implementation.
* Uses a TestApi27 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi27 backend or
* invoke operations on it. Have in mind that singals provided by TestApi27 backend should be
* emitted only by TestApi27 backend, emitting it on qml will not reach the TestApi27 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi27 : public AbstractTestApi27
{
    Q_OBJECT
...
public:
    explicit QmlTestApi27(QObject *parent = nullptr);
    ~QmlTestApi27() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi27 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi27> m_obj;
};
