...
/**
* A QML wrapper of the TestApi28 implementation.
* Uses a TestApi28 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi28 backend or
* invoke operations on it. Have in mind that singals provided by TestApi28 backend should be
* emitted only by TestApi28 backend, emitting it on qml will not reach the TestApi28 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi28 : public AbstractTestApi28
{
    Q_OBJECT
...
public:
    explicit QmlTestApi28(QObject *parent = nullptr);
    ~QmlTestApi28() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi28 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi28> m_obj;
};
