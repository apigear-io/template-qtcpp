...
/**
* A QML wrapper of the TestApi29 implementation.
* Uses a TestApi29 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi29 backend or
* invoke operations on it. Have in mind that singals provided by TestApi29 backend should be
* emitted only by TestApi29 backend, emitting it on qml will not reach the TestApi29 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi29 : public AbstractTestApi29
{
    Q_OBJECT
...
public:
    explicit QmlTestApi29(QObject *parent = nullptr);
    ~QmlTestApi29() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi29 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi29> m_obj;
};
