...
/**
* A QML wrapper of the TestApi3 implementation.
* Uses a TestApi3 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi3 backend or
* invoke operations on it. Have in mind that singals provided by TestApi3 backend should be
* emitted only by TestApi3 backend, emitting it on qml will not reach the TestApi3 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi3 : public AbstractTestApi3
{
    Q_OBJECT
...
public:
    explicit QmlTestApi3(QObject *parent = nullptr);
    ~QmlTestApi3() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi3 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi3> m_obj;
};
