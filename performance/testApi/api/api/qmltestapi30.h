...
/**
* A QML wrapper of the TestApi30 implementation.
* Uses a TestApi30 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi30 backend or
* invoke operations on it. Have in mind that singals provided by TestApi30 backend should be
* emitted only by TestApi30 backend, emitting it on qml will not reach the TestApi30 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi30 : public AbstractTestApi30
{
    Q_OBJECT
...
public:
    explicit QmlTestApi30(QObject *parent = nullptr);
    ~QmlTestApi30() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi30 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi30> m_obj;
};
