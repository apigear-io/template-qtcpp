...
/**
* A QML wrapper of the TestApi31 implementation.
* Uses a TestApi31 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi31 backend or
* invoke operations on it. Have in mind that singals provided by TestApi31 backend should be
* emitted only by TestApi31 backend, emitting it on qml will not reach the TestApi31 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi31 : public AbstractTestApi31
{
    Q_OBJECT
...
public:
    explicit QmlTestApi31(QObject *parent = nullptr);
    ~QmlTestApi31() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi31 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi31> m_obj;
};
