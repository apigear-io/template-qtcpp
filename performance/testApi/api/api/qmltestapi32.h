...
/**
* A QML wrapper of the TestApi32 implementation.
* Uses a TestApi32 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi32 backend or
* invoke operations on it. Have in mind that singals provided by TestApi32 backend should be
* emitted only by TestApi32 backend, emitting it on qml will not reach the TestApi32 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi32 : public AbstractTestApi32
{
    Q_OBJECT
...
public:
    explicit QmlTestApi32(QObject *parent = nullptr);
    ~QmlTestApi32() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi32 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi32> m_obj;
};
