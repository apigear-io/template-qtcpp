...
/**
* A QML wrapper of the TestApi33 implementation.
* Uses a TestApi33 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi33 backend or
* invoke operations on it. Have in mind that singals provided by TestApi33 backend should be
* emitted only by TestApi33 backend, emitting it on qml will not reach the TestApi33 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi33 : public AbstractTestApi33
{
    Q_OBJECT
...
public:
    explicit QmlTestApi33(QObject *parent = nullptr);
    ~QmlTestApi33() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi33 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi33> m_obj;
};
