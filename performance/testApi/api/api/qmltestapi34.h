...
/**
* A QML wrapper of the TestApi34 implementation.
* Uses a TestApi34 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi34 backend or
* invoke operations on it. Have in mind that singals provided by TestApi34 backend should be
* emitted only by TestApi34 backend, emitting it on qml will not reach the TestApi34 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi34 : public AbstractTestApi34
{
    Q_OBJECT
...
public:
    explicit QmlTestApi34(QObject *parent = nullptr);
    ~QmlTestApi34() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi34 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi34> m_obj;
};
