...
/**
* A QML wrapper of the TestApi35 implementation.
* Uses a TestApi35 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi35 backend or
* invoke operations on it. Have in mind that singals provided by TestApi35 backend should be
* emitted only by TestApi35 backend, emitting it on qml will not reach the TestApi35 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi35 : public AbstractTestApi35
{
    Q_OBJECT
...
public:
    explicit QmlTestApi35(QObject *parent = nullptr);
    ~QmlTestApi35() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi35 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi35> m_obj;
};
