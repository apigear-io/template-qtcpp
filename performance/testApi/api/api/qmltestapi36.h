...
/**
* A QML wrapper of the TestApi36 implementation.
* Uses a TestApi36 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi36 backend or
* invoke operations on it. Have in mind that singals provided by TestApi36 backend should be
* emitted only by TestApi36 backend, emitting it on qml will not reach the TestApi36 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi36 : public AbstractTestApi36
{
    Q_OBJECT
...
public:
    explicit QmlTestApi36(QObject *parent = nullptr);
    ~QmlTestApi36() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi36 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi36> m_obj;
};
