...
/**
* A QML wrapper of the TestApi37 implementation.
* Uses a TestApi37 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi37 backend or
* invoke operations on it. Have in mind that singals provided by TestApi37 backend should be
* emitted only by TestApi37 backend, emitting it on qml will not reach the TestApi37 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi37 : public AbstractTestApi37
{
    Q_OBJECT
...
public:
    explicit QmlTestApi37(QObject *parent = nullptr);
    ~QmlTestApi37() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi37 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi37> m_obj;
};
