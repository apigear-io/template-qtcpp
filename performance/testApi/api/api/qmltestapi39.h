...
/**
* A QML wrapper of the TestApi39 implementation.
* Uses a TestApi39 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi39 backend or
* invoke operations on it. Have in mind that singals provided by TestApi39 backend should be
* emitted only by TestApi39 backend, emitting it on qml will not reach the TestApi39 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi39 : public AbstractTestApi39
{
    Q_OBJECT
...
public:
    explicit QmlTestApi39(QObject *parent = nullptr);
    ~QmlTestApi39() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi39 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi39> m_obj;
};
