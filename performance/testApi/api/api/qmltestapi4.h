...
/**
* A QML wrapper of the TestApi4 implementation.
* Uses a TestApi4 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi4 backend or
* invoke operations on it. Have in mind that singals provided by TestApi4 backend should be
* emitted only by TestApi4 backend, emitting it on qml will not reach the TestApi4 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi4 : public AbstractTestApi4
{
    Q_OBJECT
...
public:
    explicit QmlTestApi4(QObject *parent = nullptr);
    ~QmlTestApi4() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi4 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi4> m_obj;
};
