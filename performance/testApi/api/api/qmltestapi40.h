...
/**
* A QML wrapper of the TestApi40 implementation.
* Uses a TestApi40 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi40 backend or
* invoke operations on it. Have in mind that singals provided by TestApi40 backend should be
* emitted only by TestApi40 backend, emitting it on qml will not reach the TestApi40 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi40 : public AbstractTestApi40
{
    Q_OBJECT
...
public:
    explicit QmlTestApi40(QObject *parent = nullptr);
    ~QmlTestApi40() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi40 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi40> m_obj;
};
