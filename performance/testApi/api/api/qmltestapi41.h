...
/**
* A QML wrapper of the TestApi41 implementation.
* Uses a TestApi41 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi41 backend or
* invoke operations on it. Have in mind that singals provided by TestApi41 backend should be
* emitted only by TestApi41 backend, emitting it on qml will not reach the TestApi41 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi41 : public AbstractTestApi41
{
    Q_OBJECT
...
public:
    explicit QmlTestApi41(QObject *parent = nullptr);
    ~QmlTestApi41() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi41 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi41> m_obj;
};
