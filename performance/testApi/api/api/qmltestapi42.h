...
/**
* A QML wrapper of the TestApi42 implementation.
* Uses a TestApi42 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi42 backend or
* invoke operations on it. Have in mind that singals provided by TestApi42 backend should be
* emitted only by TestApi42 backend, emitting it on qml will not reach the TestApi42 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi42 : public AbstractTestApi42
{
    Q_OBJECT
...
public:
    explicit QmlTestApi42(QObject *parent = nullptr);
    ~QmlTestApi42() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi42 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi42> m_obj;
};
