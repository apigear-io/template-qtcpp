...
/**
* A QML wrapper of the TestApi43 implementation.
* Uses a TestApi43 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi43 backend or
* invoke operations on it. Have in mind that singals provided by TestApi43 backend should be
* emitted only by TestApi43 backend, emitting it on qml will not reach the TestApi43 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi43 : public AbstractTestApi43
{
    Q_OBJECT
...
public:
    explicit QmlTestApi43(QObject *parent = nullptr);
    ~QmlTestApi43() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi43 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi43> m_obj;
};
