...
/**
* A QML wrapper of the TestApi44 implementation.
* Uses a TestApi44 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi44 backend or
* invoke operations on it. Have in mind that singals provided by TestApi44 backend should be
* emitted only by TestApi44 backend, emitting it on qml will not reach the TestApi44 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi44 : public AbstractTestApi44
{
    Q_OBJECT
...
public:
    explicit QmlTestApi44(QObject *parent = nullptr);
    ~QmlTestApi44() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi44 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi44> m_obj;
};
