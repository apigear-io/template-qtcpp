...
/**
* A QML wrapper of the TestApi45 implementation.
* Uses a TestApi45 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi45 backend or
* invoke operations on it. Have in mind that singals provided by TestApi45 backend should be
* emitted only by TestApi45 backend, emitting it on qml will not reach the TestApi45 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi45 : public AbstractTestApi45
{
    Q_OBJECT
...
public:
    explicit QmlTestApi45(QObject *parent = nullptr);
    ~QmlTestApi45() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi45 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi45> m_obj;
};
