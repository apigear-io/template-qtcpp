...
/**
* A QML wrapper of the TestApi46 implementation.
* Uses a TestApi46 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi46 backend or
* invoke operations on it. Have in mind that singals provided by TestApi46 backend should be
* emitted only by TestApi46 backend, emitting it on qml will not reach the TestApi46 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi46 : public AbstractTestApi46
{
    Q_OBJECT
...
public:
    explicit QmlTestApi46(QObject *parent = nullptr);
    ~QmlTestApi46() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi46 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi46> m_obj;
};
