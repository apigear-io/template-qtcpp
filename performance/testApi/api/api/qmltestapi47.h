...
/**
* A QML wrapper of the TestApi47 implementation.
* Uses a TestApi47 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi47 backend or
* invoke operations on it. Have in mind that singals provided by TestApi47 backend should be
* emitted only by TestApi47 backend, emitting it on qml will not reach the TestApi47 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi47 : public AbstractTestApi47
{
    Q_OBJECT
...
public:
    explicit QmlTestApi47(QObject *parent = nullptr);
    ~QmlTestApi47() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi47 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi47> m_obj;
};
