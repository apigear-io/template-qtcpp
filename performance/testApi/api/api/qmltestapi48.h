...
/**
* A QML wrapper of the TestApi48 implementation.
* Uses a TestApi48 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi48 backend or
* invoke operations on it. Have in mind that singals provided by TestApi48 backend should be
* emitted only by TestApi48 backend, emitting it on qml will not reach the TestApi48 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi48 : public AbstractTestApi48
{
    Q_OBJECT
...
public:
    explicit QmlTestApi48(QObject *parent = nullptr);
    ~QmlTestApi48() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi48 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi48> m_obj;
};
