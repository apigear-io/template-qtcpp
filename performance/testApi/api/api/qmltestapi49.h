...
/**
* A QML wrapper of the TestApi49 implementation.
* Uses a TestApi49 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi49 backend or
* invoke operations on it. Have in mind that singals provided by TestApi49 backend should be
* emitted only by TestApi49 backend, emitting it on qml will not reach the TestApi49 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi49 : public AbstractTestApi49
{
    Q_OBJECT
...
public:
    explicit QmlTestApi49(QObject *parent = nullptr);
    ~QmlTestApi49() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi49 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi49> m_obj;
};
