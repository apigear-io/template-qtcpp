...
/**
* A QML wrapper of the TestApi5 implementation.
* Uses a TestApi5 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi5 backend or
* invoke operations on it. Have in mind that singals provided by TestApi5 backend should be
* emitted only by TestApi5 backend, emitting it on qml will not reach the TestApi5 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi5 : public AbstractTestApi5
{
    Q_OBJECT
...
public:
    explicit QmlTestApi5(QObject *parent = nullptr);
    ~QmlTestApi5() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi5 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi5> m_obj;
};
