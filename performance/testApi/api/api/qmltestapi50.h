...
/**
* A QML wrapper of the TestApi50 implementation.
* Uses a TestApi50 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi50 backend or
* invoke operations on it. Have in mind that singals provided by TestApi50 backend should be
* emitted only by TestApi50 backend, emitting it on qml will not reach the TestApi50 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi50 : public AbstractTestApi50
{
    Q_OBJECT
...
public:
    explicit QmlTestApi50(QObject *parent = nullptr);
    ~QmlTestApi50() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi50 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi50> m_obj;
};
