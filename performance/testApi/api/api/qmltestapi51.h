...
/**
* A QML wrapper of the TestApi51 implementation.
* Uses a TestApi51 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi51 backend or
* invoke operations on it. Have in mind that singals provided by TestApi51 backend should be
* emitted only by TestApi51 backend, emitting it on qml will not reach the TestApi51 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi51 : public AbstractTestApi51
{
    Q_OBJECT
...
public:
    explicit QmlTestApi51(QObject *parent = nullptr);
    ~QmlTestApi51() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi51 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi51> m_obj;
};
