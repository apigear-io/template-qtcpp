...
/**
* A QML wrapper of the TestApi52 implementation.
* Uses a TestApi52 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi52 backend or
* invoke operations on it. Have in mind that singals provided by TestApi52 backend should be
* emitted only by TestApi52 backend, emitting it on qml will not reach the TestApi52 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi52 : public AbstractTestApi52
{
    Q_OBJECT
...
public:
    explicit QmlTestApi52(QObject *parent = nullptr);
    ~QmlTestApi52() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi52 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi52> m_obj;
};
