...
/**
* A QML wrapper of the TestApi53 implementation.
* Uses a TestApi53 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi53 backend or
* invoke operations on it. Have in mind that singals provided by TestApi53 backend should be
* emitted only by TestApi53 backend, emitting it on qml will not reach the TestApi53 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi53 : public AbstractTestApi53
{
    Q_OBJECT
...
public:
    explicit QmlTestApi53(QObject *parent = nullptr);
    ~QmlTestApi53() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi53 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi53> m_obj;
};
