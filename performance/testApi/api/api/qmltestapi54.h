...
/**
* A QML wrapper of the TestApi54 implementation.
* Uses a TestApi54 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi54 backend or
* invoke operations on it. Have in mind that singals provided by TestApi54 backend should be
* emitted only by TestApi54 backend, emitting it on qml will not reach the TestApi54 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi54 : public AbstractTestApi54
{
    Q_OBJECT
...
public:
    explicit QmlTestApi54(QObject *parent = nullptr);
    ~QmlTestApi54() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi54 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi54> m_obj;
};
