...
/**
* A QML wrapper of the TestApi55 implementation.
* Uses a TestApi55 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi55 backend or
* invoke operations on it. Have in mind that singals provided by TestApi55 backend should be
* emitted only by TestApi55 backend, emitting it on qml will not reach the TestApi55 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi55 : public AbstractTestApi55
{
    Q_OBJECT
...
public:
    explicit QmlTestApi55(QObject *parent = nullptr);
    ~QmlTestApi55() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi55 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi55> m_obj;
};
