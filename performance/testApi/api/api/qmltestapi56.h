...
/**
* A QML wrapper of the TestApi56 implementation.
* Uses a TestApi56 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi56 backend or
* invoke operations on it. Have in mind that singals provided by TestApi56 backend should be
* emitted only by TestApi56 backend, emitting it on qml will not reach the TestApi56 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi56 : public AbstractTestApi56
{
    Q_OBJECT
...
public:
    explicit QmlTestApi56(QObject *parent = nullptr);
    ~QmlTestApi56() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi56 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi56> m_obj;
};
