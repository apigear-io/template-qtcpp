...
/**
* A QML wrapper of the TestApi57 implementation.
* Uses a TestApi57 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi57 backend or
* invoke operations on it. Have in mind that singals provided by TestApi57 backend should be
* emitted only by TestApi57 backend, emitting it on qml will not reach the TestApi57 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi57 : public AbstractTestApi57
{
    Q_OBJECT
...
public:
    explicit QmlTestApi57(QObject *parent = nullptr);
    ~QmlTestApi57() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi57 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi57> m_obj;
};
