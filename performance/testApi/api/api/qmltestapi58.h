...
/**
* A QML wrapper of the TestApi58 implementation.
* Uses a TestApi58 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi58 backend or
* invoke operations on it. Have in mind that singals provided by TestApi58 backend should be
* emitted only by TestApi58 backend, emitting it on qml will not reach the TestApi58 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi58 : public AbstractTestApi58
{
    Q_OBJECT
...
public:
    explicit QmlTestApi58(QObject *parent = nullptr);
    ~QmlTestApi58() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi58 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi58> m_obj;
};
