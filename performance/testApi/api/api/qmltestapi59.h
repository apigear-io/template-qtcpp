...
/**
* A QML wrapper of the TestApi59 implementation.
* Uses a TestApi59 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi59 backend or
* invoke operations on it. Have in mind that singals provided by TestApi59 backend should be
* emitted only by TestApi59 backend, emitting it on qml will not reach the TestApi59 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi59 : public AbstractTestApi59
{
    Q_OBJECT
...
public:
    explicit QmlTestApi59(QObject *parent = nullptr);
    ~QmlTestApi59() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi59 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi59> m_obj;
};
