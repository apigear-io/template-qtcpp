...
/**
* A QML wrapper of the TestApi6 implementation.
* Uses a TestApi6 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi6 backend or
* invoke operations on it. Have in mind that singals provided by TestApi6 backend should be
* emitted only by TestApi6 backend, emitting it on qml will not reach the TestApi6 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi6 : public AbstractTestApi6
{
    Q_OBJECT
...
public:
    explicit QmlTestApi6(QObject *parent = nullptr);
    ~QmlTestApi6() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi6 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi6> m_obj;
};
