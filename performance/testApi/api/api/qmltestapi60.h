...
/**
* A QML wrapper of the TestApi60 implementation.
* Uses a TestApi60 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi60 backend or
* invoke operations on it. Have in mind that singals provided by TestApi60 backend should be
* emitted only by TestApi60 backend, emitting it on qml will not reach the TestApi60 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi60 : public AbstractTestApi60
{
    Q_OBJECT
...
public:
    explicit QmlTestApi60(QObject *parent = nullptr);
    ~QmlTestApi60() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi60 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi60> m_obj;
};
