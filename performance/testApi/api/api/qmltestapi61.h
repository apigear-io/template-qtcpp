...
/**
* A QML wrapper of the TestApi61 implementation.
* Uses a TestApi61 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi61 backend or
* invoke operations on it. Have in mind that singals provided by TestApi61 backend should be
* emitted only by TestApi61 backend, emitting it on qml will not reach the TestApi61 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi61 : public AbstractTestApi61
{
    Q_OBJECT
...
public:
    explicit QmlTestApi61(QObject *parent = nullptr);
    ~QmlTestApi61() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi61 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi61> m_obj;
};
