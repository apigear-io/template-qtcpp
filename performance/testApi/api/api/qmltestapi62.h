...
/**
* A QML wrapper of the TestApi62 implementation.
* Uses a TestApi62 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi62 backend or
* invoke operations on it. Have in mind that singals provided by TestApi62 backend should be
* emitted only by TestApi62 backend, emitting it on qml will not reach the TestApi62 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi62 : public AbstractTestApi62
{
    Q_OBJECT
...
public:
    explicit QmlTestApi62(QObject *parent = nullptr);
    ~QmlTestApi62() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi62 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi62> m_obj;
};
