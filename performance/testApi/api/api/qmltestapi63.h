...
/**
* A QML wrapper of the TestApi63 implementation.
* Uses a TestApi63 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi63 backend or
* invoke operations on it. Have in mind that singals provided by TestApi63 backend should be
* emitted only by TestApi63 backend, emitting it on qml will not reach the TestApi63 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi63 : public AbstractTestApi63
{
    Q_OBJECT
...
public:
    explicit QmlTestApi63(QObject *parent = nullptr);
    ~QmlTestApi63() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi63 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi63> m_obj;
};
