...
/**
* A QML wrapper of the TestApi64 implementation.
* Uses a TestApi64 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi64 backend or
* invoke operations on it. Have in mind that singals provided by TestApi64 backend should be
* emitted only by TestApi64 backend, emitting it on qml will not reach the TestApi64 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi64 : public AbstractTestApi64
{
    Q_OBJECT
...
public:
    explicit QmlTestApi64(QObject *parent = nullptr);
    ~QmlTestApi64() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi64 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi64> m_obj;
};
