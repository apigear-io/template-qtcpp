...
/**
* A QML wrapper of the TestApi65 implementation.
* Uses a TestApi65 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi65 backend or
* invoke operations on it. Have in mind that singals provided by TestApi65 backend should be
* emitted only by TestApi65 backend, emitting it on qml will not reach the TestApi65 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi65 : public AbstractTestApi65
{
    Q_OBJECT
...
public:
    explicit QmlTestApi65(QObject *parent = nullptr);
    ~QmlTestApi65() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi65 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi65> m_obj;
};
