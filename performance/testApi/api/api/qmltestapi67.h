...
/**
* A QML wrapper of the TestApi67 implementation.
* Uses a TestApi67 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi67 backend or
* invoke operations on it. Have in mind that singals provided by TestApi67 backend should be
* emitted only by TestApi67 backend, emitting it on qml will not reach the TestApi67 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi67 : public AbstractTestApi67
{
    Q_OBJECT
...
public:
    explicit QmlTestApi67(QObject *parent = nullptr);
    ~QmlTestApi67() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi67 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi67> m_obj;
};
