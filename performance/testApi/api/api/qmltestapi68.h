...
/**
* A QML wrapper of the TestApi68 implementation.
* Uses a TestApi68 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi68 backend or
* invoke operations on it. Have in mind that singals provided by TestApi68 backend should be
* emitted only by TestApi68 backend, emitting it on qml will not reach the TestApi68 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi68 : public AbstractTestApi68
{
    Q_OBJECT
...
public:
    explicit QmlTestApi68(QObject *parent = nullptr);
    ~QmlTestApi68() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi68 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi68> m_obj;
};
