...
/**
* A QML wrapper of the TestApi69 implementation.
* Uses a TestApi69 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi69 backend or
* invoke operations on it. Have in mind that singals provided by TestApi69 backend should be
* emitted only by TestApi69 backend, emitting it on qml will not reach the TestApi69 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi69 : public AbstractTestApi69
{
    Q_OBJECT
...
public:
    explicit QmlTestApi69(QObject *parent = nullptr);
    ~QmlTestApi69() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi69 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi69> m_obj;
};
