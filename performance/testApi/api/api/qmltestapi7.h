...
/**
* A QML wrapper of the TestApi7 implementation.
* Uses a TestApi7 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi7 backend or
* invoke operations on it. Have in mind that singals provided by TestApi7 backend should be
* emitted only by TestApi7 backend, emitting it on qml will not reach the TestApi7 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi7 : public AbstractTestApi7
{
    Q_OBJECT
...
public:
    explicit QmlTestApi7(QObject *parent = nullptr);
    ~QmlTestApi7() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi7 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi7> m_obj;
};
