...
/**
* A QML wrapper of the TestApi70 implementation.
* Uses a TestApi70 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi70 backend or
* invoke operations on it. Have in mind that singals provided by TestApi70 backend should be
* emitted only by TestApi70 backend, emitting it on qml will not reach the TestApi70 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi70 : public AbstractTestApi70
{
    Q_OBJECT
...
public:
    explicit QmlTestApi70(QObject *parent = nullptr);
    ~QmlTestApi70() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi70 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi70> m_obj;
};
