...
/**
* A QML wrapper of the TestApi71 implementation.
* Uses a TestApi71 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi71 backend or
* invoke operations on it. Have in mind that singals provided by TestApi71 backend should be
* emitted only by TestApi71 backend, emitting it on qml will not reach the TestApi71 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi71 : public AbstractTestApi71
{
    Q_OBJECT
...
public:
    explicit QmlTestApi71(QObject *parent = nullptr);
    ~QmlTestApi71() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi71 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi71> m_obj;
};
