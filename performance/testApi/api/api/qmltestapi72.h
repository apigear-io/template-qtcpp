...
/**
* A QML wrapper of the TestApi72 implementation.
* Uses a TestApi72 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi72 backend or
* invoke operations on it. Have in mind that singals provided by TestApi72 backend should be
* emitted only by TestApi72 backend, emitting it on qml will not reach the TestApi72 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi72 : public AbstractTestApi72
{
    Q_OBJECT
...
public:
    explicit QmlTestApi72(QObject *parent = nullptr);
    ~QmlTestApi72() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi72 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi72> m_obj;
};
