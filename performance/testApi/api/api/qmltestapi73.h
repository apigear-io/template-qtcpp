...
/**
* A QML wrapper of the TestApi73 implementation.
* Uses a TestApi73 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi73 backend or
* invoke operations on it. Have in mind that singals provided by TestApi73 backend should be
* emitted only by TestApi73 backend, emitting it on qml will not reach the TestApi73 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi73 : public AbstractTestApi73
{
    Q_OBJECT
...
public:
    explicit QmlTestApi73(QObject *parent = nullptr);
    ~QmlTestApi73() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi73 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi73> m_obj;
};
