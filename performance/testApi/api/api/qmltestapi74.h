...
/**
* A QML wrapper of the TestApi74 implementation.
* Uses a TestApi74 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi74 backend or
* invoke operations on it. Have in mind that singals provided by TestApi74 backend should be
* emitted only by TestApi74 backend, emitting it on qml will not reach the TestApi74 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi74 : public AbstractTestApi74
{
    Q_OBJECT
...
public:
    explicit QmlTestApi74(QObject *parent = nullptr);
    ~QmlTestApi74() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi74 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi74> m_obj;
};
