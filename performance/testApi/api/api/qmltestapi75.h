...
/**
* A QML wrapper of the TestApi75 implementation.
* Uses a TestApi75 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi75 backend or
* invoke operations on it. Have in mind that singals provided by TestApi75 backend should be
* emitted only by TestApi75 backend, emitting it on qml will not reach the TestApi75 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi75 : public AbstractTestApi75
{
    Q_OBJECT
...
public:
    explicit QmlTestApi75(QObject *parent = nullptr);
    ~QmlTestApi75() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi75 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi75> m_obj;
};
