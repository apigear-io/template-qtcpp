...
/**
* A QML wrapper of the TestApi76 implementation.
* Uses a TestApi76 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi76 backend or
* invoke operations on it. Have in mind that singals provided by TestApi76 backend should be
* emitted only by TestApi76 backend, emitting it on qml will not reach the TestApi76 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi76 : public AbstractTestApi76
{
    Q_OBJECT
...
public:
    explicit QmlTestApi76(QObject *parent = nullptr);
    ~QmlTestApi76() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi76 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi76> m_obj;
};
