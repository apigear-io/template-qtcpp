...
/**
* A QML wrapper of the TestApi77 implementation.
* Uses a TestApi77 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi77 backend or
* invoke operations on it. Have in mind that singals provided by TestApi77 backend should be
* emitted only by TestApi77 backend, emitting it on qml will not reach the TestApi77 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi77 : public AbstractTestApi77
{
    Q_OBJECT
...
public:
    explicit QmlTestApi77(QObject *parent = nullptr);
    ~QmlTestApi77() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi77 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi77> m_obj;
};
