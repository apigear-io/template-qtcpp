...
/**
* A QML wrapper of the TestApi78 implementation.
* Uses a TestApi78 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi78 backend or
* invoke operations on it. Have in mind that singals provided by TestApi78 backend should be
* emitted only by TestApi78 backend, emitting it on qml will not reach the TestApi78 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi78 : public AbstractTestApi78
{
    Q_OBJECT
...
public:
    explicit QmlTestApi78(QObject *parent = nullptr);
    ~QmlTestApi78() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi78 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi78> m_obj;
};
