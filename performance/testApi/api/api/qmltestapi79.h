...
/**
* A QML wrapper of the TestApi79 implementation.
* Uses a TestApi79 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi79 backend or
* invoke operations on it. Have in mind that singals provided by TestApi79 backend should be
* emitted only by TestApi79 backend, emitting it on qml will not reach the TestApi79 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi79 : public AbstractTestApi79
{
    Q_OBJECT
...
public:
    explicit QmlTestApi79(QObject *parent = nullptr);
    ~QmlTestApi79() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi79 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi79> m_obj;
};
