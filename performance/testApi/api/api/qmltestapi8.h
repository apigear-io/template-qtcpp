...
/**
* A QML wrapper of the TestApi8 implementation.
* Uses a TestApi8 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi8 backend or
* invoke operations on it. Have in mind that singals provided by TestApi8 backend should be
* emitted only by TestApi8 backend, emitting it on qml will not reach the TestApi8 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi8 : public AbstractTestApi8
{
    Q_OBJECT
...
public:
    explicit QmlTestApi8(QObject *parent = nullptr);
    ~QmlTestApi8() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi8 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi8> m_obj;
};
