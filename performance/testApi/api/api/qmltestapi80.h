...
/**
* A QML wrapper of the TestApi80 implementation.
* Uses a TestApi80 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi80 backend or
* invoke operations on it. Have in mind that singals provided by TestApi80 backend should be
* emitted only by TestApi80 backend, emitting it on qml will not reach the TestApi80 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi80 : public AbstractTestApi80
{
    Q_OBJECT
...
public:
    explicit QmlTestApi80(QObject *parent = nullptr);
    ~QmlTestApi80() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi80 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi80> m_obj;
};
