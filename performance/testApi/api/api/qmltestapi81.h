...
/**
* A QML wrapper of the TestApi81 implementation.
* Uses a TestApi81 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi81 backend or
* invoke operations on it. Have in mind that singals provided by TestApi81 backend should be
* emitted only by TestApi81 backend, emitting it on qml will not reach the TestApi81 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi81 : public AbstractTestApi81
{
    Q_OBJECT
...
public:
    explicit QmlTestApi81(QObject *parent = nullptr);
    ~QmlTestApi81() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi81 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi81> m_obj;
};
