...
/**
* A QML wrapper of the TestApi82 implementation.
* Uses a TestApi82 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi82 backend or
* invoke operations on it. Have in mind that singals provided by TestApi82 backend should be
* emitted only by TestApi82 backend, emitting it on qml will not reach the TestApi82 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi82 : public AbstractTestApi82
{
    Q_OBJECT
...
public:
    explicit QmlTestApi82(QObject *parent = nullptr);
    ~QmlTestApi82() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi82 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi82> m_obj;
};
