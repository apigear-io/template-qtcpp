...
/**
* A QML wrapper of the TestApi83 implementation.
* Uses a TestApi83 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi83 backend or
* invoke operations on it. Have in mind that singals provided by TestApi83 backend should be
* emitted only by TestApi83 backend, emitting it on qml will not reach the TestApi83 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi83 : public AbstractTestApi83
{
    Q_OBJECT
...
public:
    explicit QmlTestApi83(QObject *parent = nullptr);
    ~QmlTestApi83() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi83 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi83> m_obj;
};
