...
/**
* A QML wrapper of the TestApi84 implementation.
* Uses a TestApi84 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi84 backend or
* invoke operations on it. Have in mind that singals provided by TestApi84 backend should be
* emitted only by TestApi84 backend, emitting it on qml will not reach the TestApi84 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi84 : public AbstractTestApi84
{
    Q_OBJECT
...
public:
    explicit QmlTestApi84(QObject *parent = nullptr);
    ~QmlTestApi84() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi84 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi84> m_obj;
};
