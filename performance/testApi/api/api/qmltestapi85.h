...
/**
* A QML wrapper of the TestApi85 implementation.
* Uses a TestApi85 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi85 backend or
* invoke operations on it. Have in mind that singals provided by TestApi85 backend should be
* emitted only by TestApi85 backend, emitting it on qml will not reach the TestApi85 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi85 : public AbstractTestApi85
{
    Q_OBJECT
...
public:
    explicit QmlTestApi85(QObject *parent = nullptr);
    ~QmlTestApi85() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi85 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi85> m_obj;
};
