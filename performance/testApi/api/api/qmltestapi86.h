...
/**
* A QML wrapper of the TestApi86 implementation.
* Uses a TestApi86 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi86 backend or
* invoke operations on it. Have in mind that singals provided by TestApi86 backend should be
* emitted only by TestApi86 backend, emitting it on qml will not reach the TestApi86 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi86 : public AbstractTestApi86
{
    Q_OBJECT
...
public:
    explicit QmlTestApi86(QObject *parent = nullptr);
    ~QmlTestApi86() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi86 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi86> m_obj;
};
