...
/**
* A QML wrapper of the TestApi87 implementation.
* Uses a TestApi87 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi87 backend or
* invoke operations on it. Have in mind that singals provided by TestApi87 backend should be
* emitted only by TestApi87 backend, emitting it on qml will not reach the TestApi87 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi87 : public AbstractTestApi87
{
    Q_OBJECT
...
public:
    explicit QmlTestApi87(QObject *parent = nullptr);
    ~QmlTestApi87() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi87 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi87> m_obj;
};
