...
/**
* A QML wrapper of the TestApi88 implementation.
* Uses a TestApi88 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi88 backend or
* invoke operations on it. Have in mind that singals provided by TestApi88 backend should be
* emitted only by TestApi88 backend, emitting it on qml will not reach the TestApi88 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi88 : public AbstractTestApi88
{
    Q_OBJECT
...
public:
    explicit QmlTestApi88(QObject *parent = nullptr);
    ~QmlTestApi88() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi88 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi88> m_obj;
};
