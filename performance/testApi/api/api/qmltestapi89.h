...
/**
* A QML wrapper of the TestApi89 implementation.
* Uses a TestApi89 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi89 backend or
* invoke operations on it. Have in mind that singals provided by TestApi89 backend should be
* emitted only by TestApi89 backend, emitting it on qml will not reach the TestApi89 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi89 : public AbstractTestApi89
{
    Q_OBJECT
...
public:
    explicit QmlTestApi89(QObject *parent = nullptr);
    ~QmlTestApi89() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi89 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi89> m_obj;
};
