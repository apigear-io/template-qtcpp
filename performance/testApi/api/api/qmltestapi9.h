...
/**
* A QML wrapper of the TestApi9 implementation.
* Uses a TestApi9 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi9 backend or
* invoke operations on it. Have in mind that singals provided by TestApi9 backend should be
* emitted only by TestApi9 backend, emitting it on qml will not reach the TestApi9 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi9 : public AbstractTestApi9
{
    Q_OBJECT
...
public:
    explicit QmlTestApi9(QObject *parent = nullptr);
    ~QmlTestApi9() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi9 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi9> m_obj;
};
