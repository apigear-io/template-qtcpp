...
/**
* A QML wrapper of the TestApi90 implementation.
* Uses a TestApi90 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi90 backend or
* invoke operations on it. Have in mind that singals provided by TestApi90 backend should be
* emitted only by TestApi90 backend, emitting it on qml will not reach the TestApi90 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi90 : public AbstractTestApi90
{
    Q_OBJECT
...
public:
    explicit QmlTestApi90(QObject *parent = nullptr);
    ~QmlTestApi90() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi90 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi90> m_obj;
};
