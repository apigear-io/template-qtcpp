...
/**
* A QML wrapper of the TestApi91 implementation.
* Uses a TestApi91 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi91 backend or
* invoke operations on it. Have in mind that singals provided by TestApi91 backend should be
* emitted only by TestApi91 backend, emitting it on qml will not reach the TestApi91 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi91 : public AbstractTestApi91
{
    Q_OBJECT
...
public:
    explicit QmlTestApi91(QObject *parent = nullptr);
    ~QmlTestApi91() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi91 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi91> m_obj;
};
