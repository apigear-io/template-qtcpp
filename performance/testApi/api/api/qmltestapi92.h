...
/**
* A QML wrapper of the TestApi92 implementation.
* Uses a TestApi92 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi92 backend or
* invoke operations on it. Have in mind that singals provided by TestApi92 backend should be
* emitted only by TestApi92 backend, emitting it on qml will not reach the TestApi92 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi92 : public AbstractTestApi92
{
    Q_OBJECT
...
public:
    explicit QmlTestApi92(QObject *parent = nullptr);
    ~QmlTestApi92() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi92 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi92> m_obj;
};
