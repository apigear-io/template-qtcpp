...
/**
* A QML wrapper of the TestApi93 implementation.
* Uses a TestApi93 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi93 backend or
* invoke operations on it. Have in mind that singals provided by TestApi93 backend should be
* emitted only by TestApi93 backend, emitting it on qml will not reach the TestApi93 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi93 : public AbstractTestApi93
{
    Q_OBJECT
...
public:
    explicit QmlTestApi93(QObject *parent = nullptr);
    ~QmlTestApi93() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi93 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi93> m_obj;
};
