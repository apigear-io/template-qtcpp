...
/**
* A QML wrapper of the TestApi94 implementation.
* Uses a TestApi94 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi94 backend or
* invoke operations on it. Have in mind that singals provided by TestApi94 backend should be
* emitted only by TestApi94 backend, emitting it on qml will not reach the TestApi94 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi94 : public AbstractTestApi94
{
    Q_OBJECT
...
public:
    explicit QmlTestApi94(QObject *parent = nullptr);
    ~QmlTestApi94() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi94 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi94> m_obj;
};
