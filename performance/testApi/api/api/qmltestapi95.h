...
/**
* A QML wrapper of the TestApi95 implementation.
* Uses a TestApi95 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi95 backend or
* invoke operations on it. Have in mind that singals provided by TestApi95 backend should be
* emitted only by TestApi95 backend, emitting it on qml will not reach the TestApi95 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi95 : public AbstractTestApi95
{
    Q_OBJECT
...
public:
    explicit QmlTestApi95(QObject *parent = nullptr);
    ~QmlTestApi95() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi95 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi95> m_obj;
};
