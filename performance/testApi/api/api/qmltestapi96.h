...
/**
* A QML wrapper of the TestApi96 implementation.
* Uses a TestApi96 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi96 backend or
* invoke operations on it. Have in mind that singals provided by TestApi96 backend should be
* emitted only by TestApi96 backend, emitting it on qml will not reach the TestApi96 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi96 : public AbstractTestApi96
{
    Q_OBJECT
...
public:
    explicit QmlTestApi96(QObject *parent = nullptr);
    ~QmlTestApi96() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi96 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi96> m_obj;
};
