...
/**
* A QML wrapper of the TestApi97 implementation.
* Uses a TestApi97 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi97 backend or
* invoke operations on it. Have in mind that singals provided by TestApi97 backend should be
* emitted only by TestApi97 backend, emitting it on qml will not reach the TestApi97 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi97 : public AbstractTestApi97
{
    Q_OBJECT
...
public:
    explicit QmlTestApi97(QObject *parent = nullptr);
    ~QmlTestApi97() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi97 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi97> m_obj;
};
