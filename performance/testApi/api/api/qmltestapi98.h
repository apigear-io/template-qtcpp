...
/**
* A QML wrapper of the TestApi98 implementation.
* Uses a TestApi98 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi98 backend or
* invoke operations on it. Have in mind that singals provided by TestApi98 backend should be
* emitted only by TestApi98 backend, emitting it on qml will not reach the TestApi98 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi98 : public AbstractTestApi98
{
    Q_OBJECT
...
public:
    explicit QmlTestApi98(QObject *parent = nullptr);
    ~QmlTestApi98() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi98 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi98> m_obj;
};
