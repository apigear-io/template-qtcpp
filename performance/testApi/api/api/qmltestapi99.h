...
/**
* A QML wrapper of the TestApi99 implementation.
* Uses a TestApi99 backend provided by an ApiFactory.
* Use this class to easily bind to properties and signals provided by TestApi99 backend or
* invoke operations on it. Have in mind that singals provided by TestApi99 backend should be
* emitted only by TestApi99 backend, emitting it on qml will not reach the TestApi99 backend.
* See ApiFactory and factories that implement the IApiFactory for other features.
*/
class API_API_EXPORT QmlTestApi99 : public AbstractTestApi99
{
    Q_OBJECT
...
public:
    explicit QmlTestApi99(QObject *parent = nullptr);
    ~QmlTestApi99() override;
    /**
...
private:
    /**
    * Backend of AbstractTestApi99 type that provides properties on which methods will be invoked.
    * Produced by a ApiFactory factory. 
    */
	std::shared_ptr<AbstractTestApi99> m_obj;
};
