...
{{qtReturn "" $operation.Return }} {{$class}}::{{camel .Name}}({{qtParams "" $operation.Params}}) 
{
    if (!m_impl) {
        AG_LOG_WARNING(Q_FUNC_INFO + noObjectToTraceLogInfo);
    ...
    {{ if (not ($operation.Return.IsVoid)) }}return {{ end }}m_impl->{{camel .Name}}({{ qtVars $operation.Params}});
}
{{ end -}}
